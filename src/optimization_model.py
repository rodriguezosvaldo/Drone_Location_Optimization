...
def _build_base_model(docks, incidents, dock_locations_quantity, incident_to_docks, dock_to_incidents, max_dock_coverage_capacity):
    ...
    for d in docks:
        ...
        if coverable_incidents:
            ...
            model.addConstr(gp.quicksum(y[i] for i in coverable_incidents) >= x[d]) # If a dock is selected, it must cover at least one incident; otherwise x[d] = 0
        else:
            model.addConstr(x[d] == 0) # Dock covers no incidents in the dataset
...
def maximize_incidents_covered(docks, incidents, dock_locations_quantity, max_dock_coverage_capacity):
    ...
    model.setObjective(
        gp.quicksum(y[i] for i in incidents), # Maximize the number of incidents covered 
        gp.GRB.MAXIMIZE,
    )
    model.optimize()
...
def minimize_docks(docks, incidents, dock_locations_quantity, max_dock_coverage_capacity):
    incident_to_docks, dock_to_incidents, dock_distance_sum = _precompute_coverage(docks, incidents)
    ...
    # Objective function: Minimize the number of docks
    model.setObjective(
        gp.quicksum(x[d] for d in docks), # Minimize the number of docks
        gp.GRB.MINIMIZE,
    )
    model.optimize()

    if model.Status not in (gp.GRB.OPTIMAL, gp.GRB.TIME_LIMIT, gp.GRB.SUBOPTIMAL):
        print(f"Docks minimization ended with status {model.Status}")
        return
